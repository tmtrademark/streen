...
chatClient
	...
	.on('resub', (channel, username, _months, message, methods, userstate) => {
		log.warn("Resub info:", userstate);
		handleSubscription({
			channel,
			username,
	...
	.on("submysterygift", (channel, username, giftSubCount, {plan, planName}) => {
		handleSubscriptionBomb({
			channel,
			username,
			giftSubCount,
		});
	...
	.on("anonsubmysterygift", (channel, giftSubCount, {plan, planName}) => {
...
function handleSubscriptionBomb(data) {
	data.giftSubCount = parseInt(data.giftSubCount, 10);
	data.channel = data.channel.replace('#', '');
	data.ts = Date.now();
	io.to(`channel:${data.channel}`).emit('submysterygift', data);
}
